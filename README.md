...
4. Run `gen_landmark.py --net=PNet` to generate training data(Face Landmark Detection Part) for **PNet**.
5. Run `gen_merge_data.py --net=PNet` to merge two parts of training data.
6. Run `gen_tfrecords.py --net=PNet` to generate tfrecord for **PNet**.
7. After training **PNet**, run `gen_hard_example` to generate training data(Face Detection Part) for **RNet**.
8. Run `gen_landmark.py --net=RNet` to generate training data(Face Landmark Detection Part) for **RNet**.
9. Run `gen_merge_data.py --net=RNet` to merge two parts of training data.
10. Run `gen_tfrecords.py --net=RNet` to generate tfrecords for **RNet**.(**you should run this script four times to generate tfrecords of neg,pos,part and landmark respectively**)
11. After training **RNet**, run `gen_hard_example` to generate training data(Face Detection Part) for **ONet**.
12. Run `gen_landmark.py --net=ONet` to generate training data(Face Landmark Detection Part) for **ONet**.
13. Run `gen_merge_data.py --net=ONet` to merge two parts of training data.
14. Run `gen_tfrecords.py --net=ONet` to generate tfrecords for **ONet**.(**you should run this script four times to generate tfrecords of neg,pos,part and landmark respectively**)
