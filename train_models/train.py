...
def image_color_distort(inputs):
    ...
    inputs = tf.map_fn(random_hue, inputs)
    inputs = tf.map_fn(random_saturation, inputs)
    return inputs


def train(net_factory, model_save_path, max_epoch, tfrecord_path, display=200, base_lr=0.01):
    ...
    try:
        for step in range(MAX_STEP):
            ...
            if (step + 1) % display == 0:
                ...
                # landmark loss: %4f,
                print(("%s - Step: %d/%d, accuracy: %3f, cls loss: %4f, bbox loss: %4f, " +
                       "Landmark loss :%4f,L2 loss: %4f, Total Loss: %4f ,lr:%f ") % (
                        datetime.now(), step + 1, MAX_STEP, acc, cls_loss, bbox_loss,
            ...
            if i * config.BATCH_SIZE > num * 2:
                epoch = epoch + 1
                i = 0
                path_prefix = saver.save(sess, model_save_path, global_step=epoch * 2)
