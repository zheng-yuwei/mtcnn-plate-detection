...
def get_landmark_data(txt, images_dir):
    """ 从关键点txt中读取数据
    :param txt: 关键点txt文件路径
    ...
    for line in lines:
        ...
        img_path = os.path.join(images_dir, components[0]).replace('\\', '/')
        # bounding box
        bbox = [int(float(_)) for _ in components[1:5]]
        # landmark
        landmark = np.zeros((5, 2))
        for index in range(0, 5):
            landmark[index] = (float(components[5 + 2 * index]), float(components[5 + 2 * index + 1]))
...
class BBox(object):
    ...
    def project(self, point):
        """ 根据ground truth标准化左上角
        :param point: ground truth左上角坐标
        :return:
        """
